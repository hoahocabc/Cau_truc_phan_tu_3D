...
const TRANSLATIONS = {
    vi: {
        ...
        canvasMsg: "Vui lòng chọn phân tử từ menu bên trái",
        btnRotate: "Xoay Tự Động",
        btnLabels: "Nhãn Nguyên Tố",
        
        // Ghi chú
        keyLeft: "Chuột trái",
        insLeft: "Xoay góc nhìn",
        keyWheel: "Con lăn",
        insWheel: "Thu / Phóng",
        keyRight: "Chuột phải",
        insRight: "Di chuyển",
        ...
        optgroups: {
            "opt-inorganic": "Chất vô cơ",
            "opt-hydrocarbons": "Hiđrocacbon",
            "opt-halogen": "Dẫn xuất Halogen",
            "opt-alcohols": "Ancol & Phenol",
            "opt-aldehydes": "Anđehit & Xeton",
            "opt-acids": "Axit Cacboxylic",
            "opt-esters": "Este",
            "opt-amines": "Amin",
            "opt-carbs": "Cacbohiđrat",
            "opt-amino-acids": "Amino Acids"
        }
    },
    en: {
        ...
        canvasMsg: "Please select a molecule from the sidebar",
        ...
        insLeft: "Rotate View",
        keyWheel: "Scroll",
        insWheel: "Zoom In/Out",
        keyRight: "Right Click",
        insRight: "Pan View",
        ...
        optgroups: {
            "opt-inorganic": "Inorganic substances",
            "opt-hydrocarbons": "Hydrocarbons",
            "opt-halogen": "Halogen derivatives",
            "opt-alcohols": "Alcohols & Phenols",
            "opt-aldehydes": "Aldehydes & Ketones",
            "opt-acids": "Carboxylic acids",
            "opt-esters": "Esters",
            "opt-amines": "Amines",
            "opt-carbs": "Carbohydrates",
            "opt-amino-acids": "Amino Acids"
        }
    }
};
...
function setup() {
    ...
    let cWidth = container.offsetWidth || 800;
    let cHeight = container.offsetHeight || 600;
    ...
    document.getElementById('molecule-select').addEventListener('change', function() { 
        ...
        // Thêm class 'has-value' để kích hoạt CSS màu vàng
    ...
    document.getElementById('btn-save-image').addEventListener('click', captureImage);
...
function setLanguage(lang) {
    ...
    if(molSelect && molSelect.options.length > 0) {
        molSelect.options[0].innerText = t.placeholder;
    }
    ...
    setText('key-left', t.keyLeft);
    setText('ins-left-text', t.insLeft);
    
    setText('key-wheel', t.keyWheel);
    setText('ins-wheel-text', t.insWheel);
    
    setText('key-right', t.keyRight);
    setText('ins-right-text', t.insRight);
...
function captureImage() {
    ...
    if (resValue === "current") {
        ...
        const targetW = parseInt(dims[0]);
        const targetH = parseInt(dims[1]);
        let pg = createGraphics(targetW, targetH, WEBGL);
        pg.pixelDensity(1); 
        pg.textFont(myFont);
        const scaleFactor = targetH / height;
        renderScene(pg, targetW, targetH, scaleFactor, true);
...
function calculateOptimalZoom(currentAtoms) {
    ...
    if (minX === Infinity || maxX === -Infinity) return 1.5;
    if (Math.abs(maxX - minX) < 0.01 && Math.abs(maxY - minY) < 0.01) return 1.5;
    ...
    let finalZoom = Math.min(zoomX, zoomY);
    return constrain(finalZoom, 0.3, 2.5);
}
...
function drawAromaticRing(pg) {
    ...
    pg.fill(150); 
    pg.specularMaterial(50);
    pg.noStroke();
    const stepsPerDash = 10; 
    ...
    for (let i = 0; i < dashCount; i++) {
        ...
        for (let j = 0; j < stepsPerDash; j++) {
            ...
            let x1 = cos(t1) * radius;
            let y1 = sin(t1) * radius;
            let x2 = cos(t2) * radius;
            let y2 = sin(t2) * radius;
            drawCylinderBetweenPoints(pg, createVector(x1,y1,0), createVector(x2,y2,0), tubeRadius);
        }
    }
}
...
function drawBondMulti(pg, p1, p2, order) {
    ...
    pg.fill(150); 
    pg.specularMaterial(30);
    let bondRadius = 5; 
    let gap = 12; 

    if (order === 1) {
        pg.cylinder(bondRadius, dist, 24, 1); 
    } else if (order === 2) {
        pg.push(); pg.translate(gap/2, 0, 0); pg.cylinder(bondRadius, dist, 24, 1); pg.pop();
        pg.push(); pg.translate(-gap/2, 0, 0); pg.cylinder(bondRadius, dist, 24, 1); pg.pop();
    } else if (order === 3) {
        pg.cylinder(bondRadius, dist, 24, 1);
        pg.push(); pg.translate(gap, 0, 0); pg.cylinder(bondRadius, dist, 24, 1); pg.pop();
        pg.push(); pg.translate(-gap, 0, 0); pg.cylinder(bondRadius, dist, 24, 1); pg.pop();
    }
    pg.pop();
}
